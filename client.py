...
class Client:
    def __init__(self, private_key, proxy):
        ...
        self.chain_name = 'zkSync'
        self.chain_token = 'ETH'
        self.chain_id = 324
        self.proxy = proxy
        self.eip_1559 = True
        self.explorer_url = 'https://era.zksync.network/'
    ...
    async def get_balance(self, name, token_address: str="ETH") -> dict:
        """
    ...
    async def get_balance_tokens(self):
        tasks = []
        for name, token_address in TOKENS_PER_CHAIN.get(self.chain_name).items():
            if name == "ETH":
                token_address = "ETH"
            tasks.append(asyncio.create_task(self.get_balance(name, token_address)))
